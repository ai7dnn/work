...
def check_collision():
    ...
    # 패들과 충돌 체크
    if ball_center[1] >= SCREEN_HEIGHT - PADDLE_HEIGHT - BALL_RADIUS and \
       paddle.left <= ball_center[0] <= paddle.right:
        ball_speed_y = -ball_speed_y

    # 벽돌과 충돌 체크
    for block in blocks:
        if block.collidepoint(ball_center):
            blocks.remove(block)
            ball_speed_y = -ball_speed_y
            score += 10
            break
...
while running:
    ...
    # 공 이동
    ball_center[0] += ball_speed_x
    ball_center[1] += ball_speed_y
    ...
    # 게임 오버 체크
    if ball_center[1] >= SCREEN_HEIGHT:
        game_over()
        running = False
